...
const ShiftCompletionForm: React.FC<ShiftCompletionFormProps> = ({ shiftId }) => {
  ...
  const onError = (errors: any) => {
    if (errors.delivered?.type === "min") {
      alert("Bagels delivered cannot be negative.");
      return;
    }
    if (errors.pickedUp?.type === "min") {
      alert("Bagels Picked up cannot be negative.");
      return;
    }
    if (errors.minutes?.type === "min") {
      alert("Time (minutes) cannot be negative.");
      return;
    }
  ...
  return (
    <div className="flex flex-col justify-center content-center items-center w-full h-full bg-white px-9 pt-16 pb-[13.88rem] rounded-lg">
      <div className="max-w-[39.5rem] w-full min-w-0">
        ...
        <form id="ShiftCompletionForm" onSubmit={handleSubmit(onSubmit, onError)} className="flex flex-col space-y-[4.38rem]">
          <div className="flex flex-col space-y-4">
            ...
            <label>
              <input {...register("routeCompleted", { required: true,  })} className="align-middle w-6 h-6 border-2 border-[var(--Bagel-Rescue-Light-Grey,#D3D8DE)] text-blue-600 focus:ring-blue-500 appearance-none rounded-full checked:bg-blue-600 checked:border-blue-600" type="radio" value="yes" />
              <span className="pl-4 text-[var(--Bagel-Rescue-Dark-Blue,#072B68)]">Yes</span>
            </label>
            <label>
              <input {...register("routeCompleted", { required: true })} className="align-middle w-6 h-6 border-2 border-[var(--Bagel-Rescue-Light-Grey,#D3D8DE)] text-blue-600 focus:ring-blue-500 appearance-none rounded-full checked:bg-blue-600 checked:border-blue-600" type="radio" value="no" />
          ...
          <input {...register("delivered", { valueAsNumber: true, required: true, min: {value: 0, message: "Cant be negative"} })} className="h-[3.125rem] self-stretch border border-[var(--Bagel-Rescue-Light-Grey,#D3D8DE)] rounded-lg px-3" type="number"/>
          ...
          <input {...register("pickedUp", { valueAsNumber: true, required: true, min: {value: 0, message: "Cant be negative"} })} className="h-[3.125rem] self-stretch border border-[var(--Bagel-Rescue-Light-Grey,#D3D8DE)] rounded-lg px-3" type="number"/>
          <div className="flex justify-between content-center items-center">
            <label className="text-[var(--Bagel-Rescue-Dark-Blue,#072B68)] text-base font-bold">How much time did your route take?</label>
            <div className="flex h-[3.125rem] pr-[.63rem] w-[18.1875rem] min-w-0 border border-[var(--Bagel-Rescue-Light-Grey,#D3D8DE)] rounded-lg items-center">
              <input {...register("minutes", { valueAsNumber: true, required: true, min: {value: 0, message: "Cant be negative"} })} className="h-full w-5/6 rounded-lg outline-none pl-2 [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none" type="number"/>
        ...
        <div className="py-[4.44rem] flex justify-center content-center items-center">
          {!loading ? <button type="submit" form="ShiftCompletionForm" className="w-[13.125rem] h-[3.125rem] bg-[var(--Bagel-Rescue-Primary-Blue,#0F7AFF)] text-white rounded-xl font-bold">Submit</button> :
