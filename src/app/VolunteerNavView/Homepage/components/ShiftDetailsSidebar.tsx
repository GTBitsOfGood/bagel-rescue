...
const ShiftDetailsSidebar: React.FC<ShiftDetailsSidebarProps> = ({
  ...
  const [detailedShift, setDetailedShift] = useState<DetailedShiftData | null>(
    null,
  );
  const [loading, setLoading] = useState<boolean>(false);
  const [actionLoading, setActionLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [showRequestSubModal, setShowRequestSubModal] =
  ...
  const hasConfirmationForm =
    (shiftData as DetailedShiftData).confirmationForm &&
    (shiftData as DetailedShiftData).confirmationForm[dateToString(date)];
  ...
  const handleRequestSubConfirm = async () => {
    ...
    try {
      ...
      setError(
        error instanceof Error ? error.message : "failed to pick up shift",
      );
    } finally {
  ...
  const handleUndoSubConfirm = async () => {
    ...
    try {
      ...
      setError(
        error instanceof Error ? error.message : "Failed to undo sub request",
      );
    } finally {
      setActionLoading(false);
    }
    ...
    <>
      <div className="main-sidebar">
        ...
        <div className="sidebar-content">
          <h2 className="text-[24px] font-bold text-[#072B68]">
            {shiftData.routeName}
          </h2>
          ...
          {!loading && detailedShift && (
            <div className="h-[80%]">
              ...
              <div className="space-y-6 overflow-y-auto h-full [&::-webkit-scrollbar]:hidden">
                {detailedShift?.comments &&
                  Object.keys(detailedShift?.comments || {}).includes(
                    dateToString(date),
                  ) && (
                    <div
                      ...
                      style={{
                        backgroundColor:
                          "color-mix(in srgb, var(--Bagel-Rescue-Error-Red) 6%, transparent)",
                      }}
                    >
                ...
                {/* Route Locations */}
                <div>
                  <h3 className="font-semibold text-[#072B68] mb-2">
                    Route Locations
                  </h3>
                  {detailedShift?.routeInfo?.locations &&
                  detailedShift.routeInfo.locations.length > 0 ? (
                    detailedShift.routeInfo.locations.map((location, index) => (
                      <p key={index}>{location}</p>
                    ))
                  ) : (
                    <p>
                      {shiftData.area || "Location information not available"}
                    </p>
                  )}
                </div>
                ...
                <div>
                  ...
                  <p className="text-gray-600">
                    ...
                    {detailedShift?.shiftStartDate
                      ? detailedShift?.isRecurring
                        ? `${formattedDateFull(detailedShift.shiftStartDate)} - Recurring`
                        : detailedShift?.shiftEndDate
                ...
                <div>
                  ...
                  <p className="text-gray-600">
                    {formatTimeRange(
                      detailedShift.shiftStartTime,
                      detailedShift.shiftEndTime,
                    )}
                  </p>
                </div>
                  ...
                  >
                    ...
                    {!hasConfirmationForm && (
                      <svg
                        ...
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                        className="inline-block align-middle pl-1"
                      >
                        <path
                    ...
                    <>
                      <button
                        ...
                        onClick={() => {
                          ...
                          console.log(
                            "Submit confirmation form for shift:",
                            detailedShift!.id,
                          );
                        }}
