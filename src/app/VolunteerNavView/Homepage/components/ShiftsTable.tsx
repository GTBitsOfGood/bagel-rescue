 ...
 * If the `showShifts` prop is truthy, the table displays shifts, else it shows the loading bar.
 * `loadingProgress` prop is the progress % the table is loading.
...
const ShiftsTable: React.FC<ShiftsTableProps> = ({
  ...
  const [selectedShift, setSelectedShift] = useState<UserShiftData | null>(
    null
  );
  ...
  const handlePostShift = (shift: UserShiftData, e: any) => {
    e.stopPropagation();
    router.push(
      `/VolunteerNavView/ConfirmationForm?userShiftId=${
        shift.id
    ...
    <div>
      ...
      <div className={styles.tableBody}>
        {shifts.map((shift: UserShiftData) => {
          const status = shift.confirmationForms[
