...
const UserSidebar = ({
  ...
  const [draftUser, setDraftUser] = useState<UserDetails>(null);
  ...
  useEffect(() => {
    if (openInEditMode) setSidebarEditMode(true);
    let isMounted = true;
    ...
    const fetchUser = async () => {
      ...
      try {
        ...
        const normalizedUser = convertUserToDetails(result);
        setUser(normalizedUser);
        if (openInEditMode) {
          setDraftUser(structuredClone(normalizedUser));
  ...
  const startEdit = () => {
    if (!user) return;
    setDraftUser(structuredClone(user));
    setSidebarEditMode(true);
  };
  const cancelEdit = () => {
    if (!user) return;
    ...
    <div className={styles.sidebar}>
      <div className="flex justify-between w-full items-center">
        ...
        <div className="gap-4 flex">
          ...
          <button
            type="button"
            className={sidebarEditMode ? styles.doneButton : styles.editButton}
