...
export default function NewShiftPage() {
  ...
  const [isSearchingVolunteers, setIsSearchingVolunteers] =
  ...
  function locationsList() {
    ...
    return locations.map((loc) => {
      return (
        <div
          key={loc._id}
          className="content-center bg-blue-300 px-2 rounded-lg mr-2"
        >
          <p className="text-[#072B68] font-bold text-xs content-center">
            {loc["locationName"]}
          </p>
        </div>
      );
  ...
  async function saveEdits() {
    ...
    if (
      ...
      errorToast("Please enter a valid start time.");
      setIsSubmitting(false);
      return;
    }
    ...
    try {
      ...
      errorToast("Error creating shift. Please try again.");
      setIsSubmitting(false);
      return;
    } finally {
    ...
    successToast("Shift(s) created successfully.");
  }
