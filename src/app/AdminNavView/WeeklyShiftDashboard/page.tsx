...
function WeeklyShiftDashboard() {
  ...
  const [shiftToRouteMap, setShiftToRouteMap] = useState<Map<string, IRoute>>(new Map());
  const [routeToLocationsMap, setRouteToLocationsMap] = useState<Map<string, Location[]>>(new Map());
          ...
          <ShiftCard
            ...
            onOpenSidebar={async () => {
              ...
              if (shiftToRouteMap.has(String(shift["routeId"]))) {
                route = shiftToRouteMap.get(String(shift["routeId"]));
              } else {
                console.log("Searching!!!");
                route = await getRoute(shift["routeId"]);
                shiftToRouteMap.set(String(shift["routeId"]), route!);
              }

              let location_list_raw;
              if (routeToLocationsMap.has(String(shift["routeId"])))
                location_list_raw = JSON.stringify(routeToLocationsMap.get(String(shift["routeId"])));
              else {
                console.log("Searching!!!");
                location_list_raw = await getAllLocationsById(route!.locations.map((loc) => String(loc.location)));
                routeToLocationsMap.set(String(shift["routeId"]), JSON.parse(location_list_raw || "[]"));
              }

              const location_list = JSON.parse(location_list_raw || "[]").map((loc: Location) => loc.locationName + " - " + loc.area);
