...
export default function AdminAnalytics() {
  ...
  const formatMax2Decimals = (value: number) => {
  ...
  const statusColors: { [key: string]: string } = {
    Complete:
      "bg-[var(--Green-Green-100,#E3FCEF)] text-[var(--Green-Green-900,#084C29)]",
  ...
  const viewData = analyticsData
    ? {
        monthly: {
          ...
          averageShifts: formatMax2Decimals(
            Math.round(analyticsData.monthlyShiftAverage || 0),
          ),
          totalDuration: `${formatMax2Decimals((analyticsData.totalShiftDurationThisMonth || 0) / 60)} hr ${formatMax2Decimals((analyticsData.totalShiftDurationThisMonth || 0) % 60)} min`,
          averageDuration: `${Math.floor((analyticsData.averageShiftDurationThisMonth || 0) / 60)} hr ${formatMax2Decimals((analyticsData.averageShiftDurationThisMonth || 0) % 60)} min`,
        },
        yearly: {
          ...
          averageShifts: formatMax2Decimals(
            analyticsData.yearlyShiftsAverage || 0,
          ),
          totalDuration: `${Math.floor((analyticsData.totalShiftDurationThisYear || 0) / 60)} hr ${formatMax2Decimals((analyticsData.totalShiftDurationThisYear || 0) % 60)} min`,
          averageDuration: `${Math.floor((analyticsData.averageShiftDurationThisYear || 0) / 60)} hr ${formatMax2Decimals((analyticsData.averageShiftDurationThisYear || 0) % 60)} min`,
        },
      }
    : null;
  ...
  const volunteerData = analyticsData
    ? {
        ...
        volunteersWithMultipleShifts:
          analyticsData.volunteersWithMultipleShifts ?? [],
            ...
            <div className="px-9 pt-6">
              <p>
                {volunteerData.lastUpdatedAt
                  ? `Last updated on ${new Date(volunteerData.lastUpdatedAt)
            ...
            <div className="flex px-9 items-start gap-6 self-stretch">
              <div className="max-w-[26.5rem] w-full flex flex-col p-6 gap-7 items-start self-stretch rounded-lg bg-white">
                  ...
                  <div className="flex items-center bg-white rounded-lg border border-[var(--Bagel-Rescue-Light-Grey,#D3D8DE)] max-h-8 h-full">
                    ...
                    >
                      <p
                        className={
                          timeView === "monthly"
                            ? "text-[var(--Bagel-Rescue-Primary-Blue,#0F7AFF)]"
                            : "text-[var(--Bagel-Rescue-Dark-Blue,#072B68)]"
                        }
                      >
                        Monthly
                      </p>
                ...
                <div className="flex flex-col items-start gap-2 self-stretch">
                  <p className="self-stretch text-[var(--Bagel-Rescue-Dark-Blue,#072B68)] font-bold ">
                    Total Shifts
                  </p>
                  <div className="flex p-4 flex-col justify-center items-center gap-1 self-stretch rounded-2xl border-2 border-[var(--Bagel-Rescue-Light-Blue-2,#ECF2F9)]">
                    <p className="text-[var(--Bagel-Rescue-Dark-Blue,#072B68)] leading-5">
                      {overViewData.periodLabel}
                    </p>
                    <h2 className="text-[var(--Bagel-Rescue-Dark-Blue,#072B68)] font-bold text-2xl">
                      {overViewData.totalShifts}
                    </h2>
                  </div>
                  <div className="flex p-4 flex-col justify-center items-center gap-1 self-stretch rounded-2xl border-2 border-[var(--Bagel-Rescue-Light-Blue-2,#ECF2F9)]">
                    <p className="text-[var(--Bagel-Rescue-Dark-Blue,#072B68)] leading-5">
                      {overViewData.averageLabel}
                    </p>
                    <h2 className="text-[var(--Bagel-Rescue-Dark-Blue,#072B68)] font-bold text-2xl">
                      {overViewData.averageShifts}
                    </h2>
                  </div>
                </div>
                <div className="flex flex-col items-start gap-2 self-stretch">
                  ...
                  <div className="flex h-full w-full gap-3 flex-col justify-between items-start">
                    ...
                    <div className="flex w-full flex-col items-center justify-between">
                      ...
                      <div className="w-full rounded-lg border border-[var(--Bagel-Rescue-Light-Grey,#D3D8DE)]">
                              ...
                              >
                                ...
                                <div>
                                  <div className="flex justify-start text-center">
                                    <span
                                      className={`px-2 w-24 py-[.30rem] rounded-[2.75rem] text-xs font-medium ${statusColors[shift.status.charAt(0).toUpperCase() + shift.status.slice(1).toLowerCase()]}`}
                                    >
                        ...
                        {Array.from({
                          length: Math.max(
                            0,
                            10 - (analyticsData.recentShifts ?? []).length,
                          ),
                        }).map((_, index) => (
                          <div
                    ...
                    <div className="flex w-full justify-end items-center ">
                      <button
                        ...
                        type="button"
                        className="flex py-2 px-3 gap-[0.625rem] items-center border justify-center border-[var(--Bagel-Rescue-Light-Grey,#D3D8DE)] rounded-[var(--8,0.5rem)] hover:bg-[var(--Bagel-Rescue-Light-Blue-2,#ECF2F9)]"
                      >
                        <p className="text-[var(--Bagel-Rescue-Dark-Blue,#072B68)] font-medium">
                          Show More
                        </p>
                      </button>
                    </div>
              ...
              <div className="flex max-w-[17.5rem] w-full bg-white p-6 flex-col items-start gap-6 rounded-lg">
                ...
                <div className="flex flex-col items-start gap-2 self-stretch">
                  ...
                  <div className="flex p-4 flex-col justify-center items-center gap-1 self-stretch rounded-lg border-2 border-[var(--Bagel-Rescue-Light-Blue-2,#ECF2F9)]">
                    ...
                    <h2 className="text-[var(--Bagel-Rescue-Dark-Blue,#072B68)] font-bold text-2xl">
                      {volunteerData.numberOfNewVolunteers}
                    </h2>
                    {volunteerData.lastUpdatedAt ? (
                      <sub className="text-[var(--Bagel-Rescue-Text-Light,#8496B4)] font-normal leading-4">
                        &#40;
                        {`since ${new Date(
                          volunteerData.lastUpdatedAt,
                        ).toLocaleDateString("en-US", {
                  ...
                  <div className="w-full">
                    <div className="flex w-full flex-col items-start">
                      <div className="flex p-[0.625rem] justify-between items-center self-stretch">
                        <p className="text-[var(--Bagel-Rescue-Dark-Blue,#072B68)] font-bold leading-5">
                          Name
                        </p>
                        <p className="text-[var(--Bagel-Rescue-Dark-Blue,#072B68)] font-bold leading-5">
                          Date Joined
                      ...
                      <div className="flex w-full flex-col items-start self-stretch rounded-lg border border-[var(--Bagel-Rescue-Light-Grey,#D3D8DE)] last:border-b-0 ">
                        {volunteerData.newVolunteers.map(
                          (
                            volunteer: {
                              firstName: string;
                              lastName: string;
                        ...
                        {Array.from({
                          length: Math.max(
                            0,
                            5 - volunteerData.newVolunteers.length,
                          ),
                        }).map((_, index) => (
                          <div
                            key={`empty-${index}`}
                          ...
                          >
                            <p className="text-[var(--Bagel-Rescue-Text-Light,#8496B4)] font-normal leading-5">
                              &nbsp;
                            </p>
                            <p className="text-[var(--Bagel-Rescue-Text-Light,#8496B4)] font-normal leading-5">
                              &nbsp;
                ...
                <div className="flex flex-col items-start justify-between h-full w-full">
                  ...
                  <div className="flex w-full flex-col items-start">
                    ...
                    <div className="flex flex-col items-start self-stretch rounded-lg border border-[var(--Bagel-Rescue-Light-Grey,#D3D8DE)] last:border-b-0">
                      {volunteerData.volunteersWithMultipleShifts.map(
                      ...
                      {Array.from({
                        length: Math.max(
                          0,
                          5 - volunteerData.volunteersWithMultipleShifts.length,
                        ),
                      }).map((_, index) => (
                        <div
                          key={`empty-${index}`}
