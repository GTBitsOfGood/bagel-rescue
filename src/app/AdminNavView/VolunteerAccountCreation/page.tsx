...
function VolunteerAccountCreation() {
  ...
  function displayEmails() {
    return emails.map((email) => (
      <div
        key={email}
        role="button"
        className="border-slate-20 shadow-lg bg-white text-slate-800 flex w-full items-center rounded-md p-2 pl-3 transition-all hover:bg-slate-100 focus:bg-slate-100 active:bg-slate-100"
      >
        {email}
        <div className="ml-auto grid place-items-center justify-self-end">
          <button
            className="rounded-md border border-transparent p-2.5 text-center text-sm transition-all text-slate-600 hover:bg-slate-200 focus:bg-slate-200 active:bg-slate-200"
            type="button"
  ...
  async function inviteVolunteers() {
    setLoading(true);
    if (emails.length === 0) {
      errorToast("No emails to send invitations to.");
      return;
    }
    ...
    try {
      ...
      if (await response) {
        setEmails([]);
        setLoading(false);
        successToast("Invitations sent successfully!");
      }
    } catch (error) {
      ...
      errorToast("An error occurred while sending invitations.");
    }
    setLoading(false);
  }
  ...
  return (
    <div className={"flex items-center justify-center min-h-screen"}>
      <div
        className={
          "flex justify-around h-[45rem] w-5/6 bg-gray-200 shadow-lg rounded-xl"
        }
      >
        <form
          onSubmit={(e) => {
        ...
        >
          ...
          {!loading ? (
            ...
            <div className="ml-36 mt-10 content-center spinner animate-spin h-10 w-10 border-4 border-blue-500 border-t-transparent rounded-full"></div>
          )}
        </form>

        <div className="p-2 relative flex flex-col rounded-l border 0">
          <label className="rounded-md bg-slate-700 py-2 px-4 border border-transparent text-center text-white">
            Email List
          </label>
