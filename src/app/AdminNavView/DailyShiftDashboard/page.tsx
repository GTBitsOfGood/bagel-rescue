...
function DailyShiftDashboardPage() {
  ...
  const [routeToLocationsMap, setRouteToLocationsMap] = useState<
    Map<string, Location[]>
  >(new Map());
  const [dailyShiftsLoading, setDailyShiftsLoading] = useState(true);
  const { loadingProgress, showContent: showShifts } =
    useLoadingProgress(dailyShiftsLoading);
      ...
      try {
        ...
        dailyShiftData = dailyShiftData.filter((shift: any) => {
          return !shift.canceledShifts
            .map((canceledShift: Date) => dateToString(new Date(canceledShift)))
