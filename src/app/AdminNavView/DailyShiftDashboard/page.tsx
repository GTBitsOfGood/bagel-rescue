...
import { getShiftsByDay } from '@/server/db/actions/shift';
...
function DailyShiftDashboardPage() {
  ...
  useEffect(() => {
    const fetchDailyShifts = async (targetDate: Date) => {
      try {
        ...
        const dailyShiftData = JSON.parse(dailyShiftResponse || "[]");
  ...
  const shiftCardsList = () => {
    return dailyShiftData.map((shift: any, shiftIndex) => {
      // Filter by status
  ...
  const assignedCount = dailyShiftData.filter((shift: any) => shift.status === "assigned").length;
  const openCount = dailyShiftData.filter((shift: any) => shift.status === "open").length;
